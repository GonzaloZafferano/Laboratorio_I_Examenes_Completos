 ...
 * \return INT. Retorna -1 (Error) si el puntero a la lista o a la funcion es NULL, o
                mayor a 0, que es la cantidad de elementos que se pudieron mapear.
 */
 ...
 * \param LinkedList* this - Puntero a la lista
 * \param int(*pFunc)(void*) - Puntero a la funcion criterio
 * \return int Retorna NULL Si ocurrio un ERROR, caso contrario, retorna la direccion de
 *             memoria de una nueva LinkedList.
 */
LinkedList* ll_filterSinAlterarListaOriginal(LinkedList* this, int (*pFunc)(void*))
{
	int i;
	void* pAuxiliar;
	int retornoFuncionCriterio;
	...
	if(this != NULL && pFunc != NULL)
	{
		...
		if(listaAuxiliar != NULL)
		{
			for(i = 0; i < ll_len(this); i++)
			{
				...
				if(!retornoFuncionCriterio)
				{
					ll_add(listaAuxiliar, pAuxiliar);
				}
			}
		}
	}
